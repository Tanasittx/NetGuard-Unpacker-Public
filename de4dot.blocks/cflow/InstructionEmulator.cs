...
namespace de4dot.blocks.cflow {
	public class InstructionEmulator {
		...
		void Emulate_Ldsfld(Instruction instr) {
            ...
            FieldDef fie = instr.Operand as FieldDef;
			if (fie != null) {
				if (valueStack.fieldsDictionary.TryGetValue(fie, out val)) {
					valueStack.Push(val);
				}
				else {
		...
		void Emulate_Add(Instruction instr) {
			...
			else if (val1.IsInt32() && val2.IsReal8())
			    valueStack.Push(Real8Value.Add((Int32Value)val1, (Real8Value)val2));
			else if (val1.IsReal8() && val2.IsInt32())
			    valueStack.Push(Real8Value.Add((Real8Value)val1, (Int32Value)val2));
            else
				valueStack.PushUnknown();
		}
